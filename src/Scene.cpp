...
Scene Scene::jeep() {
    auto jeep = assimpLoad("./assets/models/mil_jeep_fbx/chassis.fbx", true);

    jeep.addChild(assimpLoad("./assets/models/mil_jeep_fbx/b_l_tire_centered.fbx", true));
    jeep.addChild(assimpLoad("./assets/models/mil_jeep_fbx/b_r_tire_centered.fbx", true));
    jeep.addChild(assimpLoad("./assets/models/mil_jeep_fbx/f_l_tire_centered.fbx", true));
    jeep.addChild(assimpLoad("./assets/models/mil_jeep_fbx/f_r_tire_centered.fbx", true));
    ...
    auto cactus = assimpLoad("./assets/models/desert/Cactus.fbx", true);
    auto cactusMesh = cactus.getChild(0).getMesh(0);
    cactusMesh.addTexture(Texture::loadTexture("./assets/models/desert/textures/tex_cactus01.png", "baseTexture"));
    cactus.move(glm::vec3(0, -0.5, 5));
    cactus.grow(glm::vec3(0.01, 0.01, 0.01));

    auto bird = assimpLoad("./assets/models/bird/duck.fbx", true);
    bird.grow(glm::vec3(0.01, 0.01, 0.01));

    std::vector<Texture> groundtex = {Texture::loadTexture("./assets/models/concrete.png","baseTexture"),Texture::loadTexture("./assets/models/test_specular.jpg","specMap")};
    //std::vector<Mesh3D> ground = {Mesh3D::square(groundtex)};
    Object3D map = Object3D({Mesh3D::square(groundtex)});//assimpLoad("./assets/models/racetrack/arena.obj", true);
    map.grow(glm::vec3(100,100,100));
    map.move(glm::vec3(0,-1,0));
    map.rotate(glm::vec3(glm::radians(-90.0f),0,0));

    std::vector<Texture> walltex = {Texture::loadTexture("./assets/models/wall.jpg","baseTexture")};

    Object3D map2 = Object3D({Mesh3D::square(walltex)});//assimpLoad("./assets/models/racetrack/arena.obj", true);
    map2.grow(glm::vec3(100,15,10));
    map2.move(glm::vec3(0,-1.2,50));
    map.rotate(glm::vec3(0,glm::radians(180.0),0));
    //map.rotate(glm::vec3(glm::radians(-90.0f),0,0));
    auto lightSource = assimpLoad("./assets/models/tiger/scene.gltf", true);
    ...
    std::vector<Texture> groundtex2 = {
            Texture::loadTexture("./assets/models/desert_pbr_textures/desert-rocks1-albedo.png","baseTexture"),
            Texture::loadTexture("./assets/models/desert_pbr_textures/desert-rocks1-Roughness.png","specMap"),
            Texture::loadTexture("./assets/models/desert_pbr_textures/desert-rocks1-Height.png","heightMap"),
            Texture::loadTexture("./assets/models/desert_pbr_textures/desert-rocks1-Normal-ogl.png","normalMap")};
    for (int x = -5; x < 5; x++) {
        for (int z = -5; z < 5; z++) {
            //,
            //int x = 0;
            ...
            Object3D groundPanel = Object3D({Mesh3D::square(groundtex2)});//assimpLoad("./assets/models/racetrack/arena.obj", true);
...
Scene Scene::lifeOfPi() {
    ...
    auto boat = assimpLoad("./assets/models/boat/boat.fbx", true);
    boat.move(glm::vec3(0, -0.7, 0));
    boat.grow(glm::vec3(0.01, 0.01, 0.01));
    auto tiger = assimpLoad("./assets/models/tiger/scene.gltf", true);
 ...
 */
Scene bunny() {
    auto bunny = assimpLoad("./assets/models/bunny_textured.obj", true);
...
Scene marbleSquare() {
    std::vector<Texture> textures = {
            Texture::loadTexture("./assets/models/White_marble_03/Textures_4K/white_marble_03_4k_baseColor.tga", "baseTexture"),
    };
